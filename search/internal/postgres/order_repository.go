...
func (r OrderRepository) Get(ctx context.Context, orderID string) (*models.Order, error) {
	...
	var itemData []byte
	err := r.db.QueryRowContext(ctx, r.table(query), order.OrderID).Scan(&order.CustomerID, &order.CustomerName, &itemData, &order.Status, order.CreatedAt)
	if err != nil {
		return nil, err
	}

	var items []models.Item
	err = json.Unmarshal(itemData, &itemData)
	if err != nil {
		return nil, err
	}
	order.Items = items
...
func (a *IDArray) Scan(src any) error {
	...
	case []byte:
		data = v
	default:
		return errors.ErrInvalidArgument.Msgf("IDArray: unsupport type: %T", src)
	}
