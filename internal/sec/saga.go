...
import "github.com/esmaeel67/golang-modular-app/internal/am"

const (
	SagaCommandIDHdr   = am.CommandHdrPrefix + "SAGA_ID"
	SagaCommandNameHdr = am.CommandNameHdr + "SAGA_NAME"

	SagaReplyIDHdr   = am.ReplyHdrPrefix + "SAGA_ID"
	SagaReplyNameHdr = am.ReplyNameHdr + "SAGA_NAME"
)
...
func (s *saga[T]) AddStep() SagaStep[T] {
	step := &sagaStep[T]{
		actions: map[bool]StepActionFunc[T]{
			notCompensating: nil, // The action/function that performs the compensation (the "undo" operation)
			isCompensating:  nil, // A flag or indicator showing whether the compensation is currently in progress or has been triggered
		},
