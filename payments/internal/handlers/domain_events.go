...
func NewDomainHandlers(publisher am.MessagePublisher[ddd.Event]) ddd.EventHandler[ddd.Event] {
...
func RegisterDomainEventHandlers(subscriber ddd.EventSubscriber[ddd.Event], handlers ddd.EventHandler[ddd.Event]) {
	subscriber.Subscribe(handlers, models.InvoicePaidEvent)
}
...
func (h domainHandlers[T]) onInvoicePaid(ctx context.Context, event ddd.Event) error {
	...
	return h.publisher.Publish(ctx, paymentspb.InvoiceAggregateChannel, ddd.NewEvent(paymentspb.InvoicePaidEvent, &paymentspb.InvoicePaid{
		Id:      payload.ID,
