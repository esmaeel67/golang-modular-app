...
func (h EventHandlers[T]) HandleEvent(ctx context.Context, event T) (err error) {
	messageIn := fmt.Sprintf("--> Notifications.%s.On(%s)", h.label, event.EventName())
	h.logger.Info(logger.Notifications, logger.HandleEvent, messageIn, nil)
	defer func() {
		messageOut := fmt.Sprintf("<-- Notifications.%s.On(%s)", h.label, event.EventName())
		h.logger.Info(logger.Notifications, logger.HandleEvent, messageOut, nil)
	}()
